...
// Vérifier si l'ID de la fête est présent dans l'URL (via GET) et s'assurer qu'il n'est pas vide
if (isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
    // Préparer une requête SQL pour récupérer les détails de la fête spécifique
    // La requête joint les tables 'events' et 'users' pour récupérer des informations supplémentaires sur l'utilisateur qui a créé la fête
    ...
    if ($stmt = $pdo->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            // Vérifier si la requête a retourné une ligne, ce qui signifie que la fête existe
            if ($stmt->rowCount() == 1) {
                // Récupérer les données de la fête sous forme de tableau associatif
                $event = $stmt->fetch(PDO::FETCH_ASSOC);
            } else {
                // Si auc la fête n'est trouvé avec cet ID, rediriger vers une page d'erreur
    ...
    // Rediriger vers une page d'erreur si l'ID de la fête n'est pas spécifié dans l'URL
    header("location: error.php");
    exit();
}
?>
