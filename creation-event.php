...
$is_public = 1; // Valeur par défaut pour la fête public
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($title_err) && empty($description_err) && empty($event_date_err) && empty($location_err) && empty($image_err)) {
        ...
        if ($stmt = $pdo->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                // Rediriger vers la liste desa fêtes ou une page de confirmation
...
<body>
    ...
    <div class="container">
        <h2>Créer un Nouvela fête</h2>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
            <div>
                <label>Titre du lea fête</label>
